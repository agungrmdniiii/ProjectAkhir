    ...
    <body class="custom-content pest-detection-page">
        ...
        <script>
            $(document).ready(function () {
                ...
                $.ajax({
                    url: '{{ route("get.persebaran.hama") }}',
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        console.log('Data persebaran hama:', data); // Debug log

                        if (data.length === 0) {
                            $('#persebaran-map').html('<p class="text-center">Belum ada data persebaran hama</p>');
                            return;
                        }
                        ...
                        // Kelompokkan data berdasarkan nama hama langsung
                        data.forEach(item => {
                        ...
                        console.log('Data yang dikelompokkan:', hamaGroups); // Debug log

                        // Buat marker untuk setiap jenis hama
                        Object.keys(hamaGroups).forEach(namaHama => {
                            const markers = hamaGroups[namaHama].map((item, index) => {
                                // Tambahkan offset kecil untuk setiap marker agar tidak tertumpuk
                                const lat = parseFloat(item.koordinat_lat) + (index * 0.0001);
                                const lon = parseFloat(item.koordinat_lon) + (index * 0.0001);

                                const popupContent = `
                                    <div>
                                        <h6>${item.jenis_hama}</h6>
                                        <p><strong>Lokasi:</strong> ${item.lokasi}</p>
                                        <p><strong>Waktu:</strong> ${new Date(item.waktu_pelaporan).toLocaleString('id-ID')}</p>
                        ...
                        // Jika ada data, fokus peta ke marker pertama
                        if (data.length > 0) {
                            const firstItem = data[0];
                            persebaranMap.setView([firstItem.koordinat_lat, firstItem.koordinat_lon], 13);
                        }
                    },
                    error: function(error) {
